// pages/smiling/skillDetail/index.js
...
Page({
  ...
  getArticleDetail(){
    var id = this.data.id;
    console.log(id, '传')
    app.post(app.globalData.ServerURL + 'technicalgrade/detail', {
      'id': id
    }).then((res) => {
      console.log(res, '文章详情');
      if (res.data.code == 0) {
        if (res.data.data.technicalgrade){
          var articleDetailData = res.data.data.technicalgrade;
        ...
        console.log(this.data.articleDetailData, '数据')
      }
    }).catch((errMsg) => {
      console.log(errMsg);
    });
